...
**Note:** An App Token is optional but recommended. Without one, requests share the portal's per-IP pool and may be throttled. Register at [https://dev.socrata.com/register](https://dev.socrata.com/register), or generate one from a portal's *Developer Settings → App Tokens*. Leave `app_token` unset rather than guessing: some portals (e.g. data.cdc.gov) serve untokened requests fine but reject an invalid token with HTTP 403.
