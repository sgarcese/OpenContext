...
class TestPluginInitialization:
    ...
    async def test_plugin_runs_without_app_token_and_sends_no_header(
        ...
        """An app token is optional: portals such as data.cdc.gov serve
        untokened requests but reject an invalid token (403), so a missing or
        blank token must mean "send no X-App-Token header", not "refuse to
        start"."""
