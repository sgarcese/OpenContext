...
class SocrataPluginConfig(BasePluginConfig):
    ...
    app_token: str | None = Field(
        None,
        description=(
            "Socrata App Token (optional). Without one, requests share the "
            "portal's per-IP pool and may be throttled; portals such as "
            "data.cdc.gov serve untokened requests but reject an invalid "
            "token (403), so leave this unset rather than guessing. Must be an App "
    ...
    def validate_app_token(cls, v: str | None) -> str | None:
        """Normalize the app token: a blank value means "no token" (send no header)."""
